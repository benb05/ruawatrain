...
class MazeSolver
{
  ...
  public char[][] _maze, _pinkyMap, _inkyMap, _blinkyMap, _clydeMap;
  ...
  public MazeSolver( String inputFile )
  {
    ...
    _pinkyMap = new char[80][25];
    _inkyMap = new char[80][25];
    _blinkyMap = new char[80][25];
    _clydeMap = new char[80][25];
    h = 0;
    w = 0;
    ...
    try {
      ...
      while( sc.hasNext() ) {
        ...
        for( int i=0; i<line.length(); i++ )
        {
          ...
          _pinkyMap[i][row] = line.charAt( i );
          _inkyMap[i][row] = line.charAt( i );
          _blinkyMap[i][row] = line.charAt( i );
          _clydeMap[i][row] = line.charAt( i );
        }
        h++;
        row++;
      }
      ...
      {
        _maze[i][row] = WALL;
        _pinkyMap[i][row] = WALL;
        _inkyMap[i][row] = WALL;
        _blinkyMap[i][row] = WALL;
        _clydeMap[i][row] = WALL;
      }
      h++;
      row++;
   ...
   **/
  public void solve( int x, int y, char[][] map )
  {
    ...
    else if ( map[x][y] != '#' ) {  // VERY IFFY PLEASE CHECK MAYBE AT????
      return;
    }
...
public class Maze
{
  public static void main( String[] args )
  {
    ...
    ms.solve( startX, startY, ms._maze );
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
  }//end main()
